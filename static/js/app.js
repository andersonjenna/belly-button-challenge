...
function buildMetadata(sample) {
  d3.json("https://static.bc-edx.com/data/dl-1-2/m14/lms/starter/samples.json").then((data) => {
    ...
    let filteredMetadata = metadata.filter(m => m.id == sample);
...
function buildCharts(sample) {
  d3.json("https://static.bc-edx.com/data/dl-1-2/m14/lms/starter/samples.json").then((data) => {
    ...
    let result = samples.filter(s => s.id == sample);
    let selectedSample = result[0];
    ...
    let top10OtuIds = otu_ids.slice(0, 10).map(id => `OTU ${id}`);
